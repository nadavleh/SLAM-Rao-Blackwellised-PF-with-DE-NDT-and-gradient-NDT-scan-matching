%% Scan Matching version 7: trying DE
% here i need to try to run the Differential evolution optimizer from:
% SLAM\SLAM by me\Differential Evolution (DE) and-or Scan Matching - working functions\optimizer-master"
% which i found on Github. add this folder to the patch and try running
% this version.


% the results here are the same as optimizing with fmincon() (the matlab
% multivariate gradient based optimizer), so i can deduce there is
% something wrong with my state updating using the result of the
% translation-rotation vector from the optimizer (because ive seen in the
% example in version 6, that the cost function optimizationm is good
% because it matched nicely two scans which are 10 timesteps from one
% another)
...
Nm=1; %Number of global maps, each on with Np particels
threshold=0.9;
...
lb = [trans_vec(1)-0.1, trans_vec(2)-0.1, trans_vec(3)-0.1];
ub = [trans_vec(1)+0.1, trans_vec(2)+0.1, trans_vec(3)+0.1]; 
